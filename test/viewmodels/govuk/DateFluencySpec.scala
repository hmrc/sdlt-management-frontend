...
class DateFluencySpec
  ...
  ".apply" - {
    ...
    "must highlight the day field when the error is that a day is missing" in {
      ...
      result.items.find(_.id == "value.day").value.classes must include(
        errorClass
      )
      result.items
    ...
    "must highlight the day and month fields when the error is that a day and month are both missing" in {
      ...
      result.items.find(_.id == "value.month").value.classes must include(
        errorClass
      )
      result.items
        .find(_.id == "value.year")
    ...
    "must highlight the day and year fields when the error is that a day and year are both missing" in {
      ...
      result.items
        ...
        .value
        .classes must not include errorClass
      result.items.find(_.id == "value.year").value.classes must include(
        errorClass
      )
    }
