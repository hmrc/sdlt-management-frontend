...
class TestControllerToBeDeleted @Inject()(
  ...
  def onPageLoad(storn: String): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      for {
        getAllInProgressReturns         <- stampDutyLandTaxService.getInProgressReturnsViewModel(None)
        ...
        Ok(Html(
          s"""
             ...
             |<br><br>
             |inProgressReturnsDueForDeletion: $inProgressReturnsDueForDeletion
             |<br><br>
             |submittedReturnsDueForDeletion: $submittedReturnsDueForDeletion
             |""".stripMargin
        ))
      }
  }
